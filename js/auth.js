...
// Sneakers Data (embedded JSON - same as home.js)
const sneakersData = [
  {
    "id": 1,
    "name": "Air Max 270",
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Show login form by default on auth page
    showLoginForm();
});
...
function setupEventListeners() {
    ...
    // Phone Number Validation - Allow numbers, +, spaces, hyphens, and parentheses
...
function navigateToPage(page) {
    // For multi-page navigation, redirect to the actual page
...
function openAuth() {
    // For auth.html, we're already on the auth page
    // For other pages, open the modal
    document.getElementById('authPopup').classList.add('active');
    showLoginForm();
}

function closeAuth() {
    // If we're on auth.html, redirect to home
...
function showLoginForm() {
    // Handle both the page form and modal form
    const loginFormPage = document.getElementById('loginForm');
    const signupFormPage = document.getElementById('signupForm');
    const loginFormModal = document.getElementById('loginFormModal');
    const signupFormModal = document.getElementById('signupFormModal');
    
    if (loginFormPage && signupFormPage) {
        loginFormPage.style.display = 'block';
        signupFormPage.style.display = 'none';
    }
    
    if (loginFormModal && signupFormModal) {
...
function handleLogin(e) {
    ...
    // Get the correct form elements based on which form was submitted
    let email, password, errorDiv;
    ...
    if (e.target.id === 'loginForm' || e.target.closest('#loginForm')) {
        ...
        // Modal form
        email = document.getElementById('loginEmailModal')?.value || '';
        password = document.getElementById('loginPasswordModal')?.value || '';
        errorDiv = document.getElementById('loginErrorModal');
    }
    ...
    if (user) {
        ...
        // Redirect to home page if on auth.html
        if (window.location.pathname.includes('auth.html')) {
            window.location.href = 'index.html';
        } else {
            closeAuth();
...
function handleSignup(e) {
    ...
    // Get the correct form elements based on which form was submitted
    let name, email, password, confirmPassword, errorDiv;
    
    if (e.target.id === 'signupForm' || e.target.closest('#signupForm')) {
        // Page form
        name = document.getElementById('signupName')?.value || '';
        email = document.getElementById('signupEmail')?.value || '';
        password = document.getElementById('signupPassword')?.value || '';
        confirmPassword = document.getElementById('signupConfirmPassword')?.value || '';
        errorDiv = document.getElementById('signupError');
    } else {
        // Modal form
        name = document.getElementById('signupNameModal')?.value || '';
        email = document.getElementById('signupEmailModal')?.value || '';
        password = document.getElementById('signupPasswordModal')?.value || '';
        confirmPassword = document.getElementById('signupConfirmPasswordModal')?.value || '';
        errorDiv = document.getElementById('signupErrorModal');
    }
...
function proceedToCheckout() {
    ...
    // Pre-fill shipping info if saved
    if (shippingInfo) {
...
function renderShippingInfo() {
    ...
    // Add shipping info section if user is logged in
